"""CLI быстрой расшифровки (лежит в корне проекта — стабильный Windows ProcessPool spawn)."""
...
def _acquire_output_lock(output_dir: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Дождитесь его завершения; повторный --fresh запуск удалит checkpoints."
        ) from exc
...
def _pool_job(payload: tuple):
    """Выполняется в child: импорт пакета уже внутри процесса."""
    import pool_decode_worker

    return pool_decode_worker.decode_one(payload)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.fresh:
        for name in (
            "profiles_long.csv", "profiles_working.csv", "profile_metrics.csv",
            "decoded_levels.csv", "decoded_all_levels.csv",
            "debufr_elements.csv", "monthly_summary.csv", "station_summary.csv",
            "summary.json", "field_types.csv", "profile_climate.xlsx",
            "aldan_actual.xlsx", "scanned_sources.csv",
        ):
    ...
    logger.info(
        ...
        station_name, station_id, len(files), len(files) - len(pending), len(pending), args.workers,
    )
    ...
    config_info = {
        "station_id": station_id, "station_slug": station_slug, "station_name": station_name,
        "start_date": start.isoformat(), "end_date": end.isoformat(),
        "pressure_top_hpa": pc_cfg.pressure_top_hpa, "cycles": cycles,
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        for batch_start in range(0, len(pending), batch_size):
            ...
            futures = {
                pool.submit(_pool_job, (str(path),) + worker_args): path
                for path in batch
            }
            for future in as_completed(futures):
                ...
                if processed % args.checkpoint_every == 0 or processed == len(pending):
                    ...
                    logger.info(
                        ...
                        processed, len(pending), len(metrics_rows), len(long_rows),
                        decoded_streamed, elements_streamed, found, errors,
                    )
    ...
    print(json.dumps({
        "station_id": station_id, "files_total": len(files), "files_pending": len(pending),
