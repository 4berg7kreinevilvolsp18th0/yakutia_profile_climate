...
- по умолчанию сохраняются **все исходные уровни** из `profiles_long.csv` в диапазоне 500–1000 гПа;
- профили со статусами QC и одиночные уровни не отбрасываются при сборке JSON;
- скачки, дубли и немонотонные участки видны для ручной оценки;
- переключатель «Подготовить кривые» включает очистку только при отображении.
...
Эти правила **печатаются на каждом PNG** (блок «Фильтры PNG») и дублируются в `params.json` (`filters_human`). Они не применяются автоматически к дашборду.
...
| `inversion_detected` | Подтверждённая приземная инверсия (v2: рост T + устойчивое падение выше верха) |
...
## Алгоритм инверсии (v2)

1. Уровни от нижнего до 500 гПа, сортировка по давлению (земля → верх).
2. **Этап 1 (кандидат):** идём вверх, пока \(T\) растёт сильнее `min_inversion_delta_c` (по умолчанию **0.2 °C**); верх = последний растущий уровень.
3. **Этап 2 (подтверждение):** выше верха — `confirm_drop_levels` (по умолчанию **2**) шага с падением \(\Delta T \le -\)`min_drop_delta_c`, и окно толщиной ≥ `confirm_depth_hpa` (**30** гПа) с суммарным падением \(T\).
4. `inversion_detected = true` **только** при подтверждении; иначе при наличии кандидата — `rejected_no_lapse` (зубец / «загон»), без засчитывания.
5. Полные формулы, примеры и описание дашборда: [`docs/METHODS_HEIGHT_INVERSION.md`](docs/METHODS_HEIGHT_INVERSION.md).
...
- Высота уровней: obs / Φ→z / интерп. по P / барометрия (см. methods-doc).
- Исторические extract до обновления кода могли считать инверсию по v1.

## Дальнейшее развитие

- Влажность, ветер
- Seasonal climatology и тренды высоты инверсии
- Сравнение Алдан / Якутск
- Re-extract метрик под v2 для климатических рядов
